...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if (fDebug && GetBoolArg("-printcoinstake")) {
		printf("!!!! nBalance = %ld\n", nBalance);
		printf("!!!! nReserveBalance = %ld\n", nReserveBalance);
		printf("!!!! txNew.nTime = %ld\n", txNew.nTime);

	}

    if (!SelectCoinsSimple(nBalance - nReserveBalance, txNew.nTime, nCoinbaseMaturity + 10, setCoins, nValueIn)) {
    	if (fDebug && GetBoolArg("-printcoinstake")) { printf("!!!! !SelectCoinsSimple");}
        return false;
	}

    if (setCoins.empty()) {
    	if (fDebug && GetBoolArg("-printcoinstake")) { printf("!!!! empty setCoins");}
        return false;
	}
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && !fShutdown && pindexPrev == pindexBest; n++)
        {
            ...
            if (fDebug && GetBoolArg("-printcoinstake")) { printf("!!!! Before CheckStakeKernelHash\n");}
            ...
            {
            	if (fDebug && GetBoolArg("-printcoinstake")) { printf("!!!! Failed CheckStakeKernelHash\n");}
            }
    ...
    // Calculate coin age reward
    {
		...
		printf("!!!! Coin age %ld", nCoinAge);

        int64_t nReward = GetProofOfStakeReward(nCoinAge, nFees, pindexPrev->nMoneySupply, pindexPrev->nHeight + 1);
        if (nReward <= 0)
            return false;
