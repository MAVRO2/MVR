...
static bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval)
    {
    	if (fDebug && GetBoolArg("-printcoinstake")) {
    		printf("\n\n");
    		printf("!!!! height = %d\n", pindex->nHeight);
    		printf("!!!! nStakeModifierTime              = %ld\n", nStakeModifierTime);
    		printf("!!!! pindexFrom->GetBlockTime()      = %ld\n", pindexFrom->GetBlockTime());
    		printf("!!!! nStakeModifierSelectionInterval = %ld\n", nStakeModifierSelectionInterval);
    		printf("!!!! GetBlockTime()+Interval         = %ld\n", pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval);
    	}

        if (!pindex->pnext)
        {   // reached best block; may happen if node is behind on block chain
        	if (fDebug && GetBoolArg("-printcoinstake")) { printf("!!!! Best block\n");}
        ...
        if (pindex->GeneratedStakeModifier())
        {
        	if (fDebug && GetBoolArg("-printcoinstake")) {printf("!!!! Stake modifier\n");}
...
bool CheckStakeKernelHash(unsigned int nBits, const CBlock& blockFrom, unsigned int nTxPrevOffset, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, uint256& targetProofOfStake, bool fPrintProofOfStake)
{
    ...
    if (!GetKernelStakeModifier(hashBlockFrom, nStakeModifier, nStakeModifierHeight, nStakeModifierTime, fPrintProofOfStake)) {
    	if (fDebug && GetBoolArg("-printcoinstake")) { printf("!!!! Failed GetKernelStakeModifier\n");}
        return false;
	}
    ss << nStakeModifier;
