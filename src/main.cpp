...
unsigned int nStakeMinAge = 10 * 60;//60 * 60 * 8; //!!!!!!! 8 hours
unsigned int nStakeMaxAge = 60 * 60 * 24 * 7; // 7 days        
unsigned int nModifierInterval = 1 * 60;// !!!! 20 * 60; // time to elapse before new modifier is computed
...
const string strMessageMagic = "MVR Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int64_t nMoneySupply, int nHeight)
{
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        int64_t nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, nFees, pindex->pprev->nMoneySupply, pindex->nHeight);
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    {
    	if (fDebug && GetBoolArg("-printcoinstake")) { printf("!!!! Before createcoinstake\n");}
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MVR", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "MVRCoin Genesis Block";
        CTransaction txNew;
        if (!fTestNet)
        {
            txNew.nTime =1492538540;
        }
        else
        {
        ...
        // debug print
        assert(block.hashMerkleRoot == uint256("0xee19e88fc54701b89d776715b1846676665a2c18b280d7fe5e591620edc63b28"));
        block.print();

//		for (unsigned long ii = 158392; ii < UINT64_MAX; ii++) {
// 			block.nNonce = ii;
// 			printf("Ii = %lu\n", ii);
// 			printf("block.GetHash() == %s\n", block.GetHash().ToString().c_str());
...
unsigned char pchMessageStart[4] = { 0xe1, 0xed, 0xe4, 0xe3 };
